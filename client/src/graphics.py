...
class GraphicsEngineClass():
    ...
    def create_display(self, size, flags=0):
        ...
        self.sky_image = pygame.transform.scale(
            pygame.image.load("..\\data\\textures\\packs\\" + self.texture_dir + "\\sky.png"), self.screen.get_size())
        try:
    ...
    def draw_screen(self):
        ...
        if self.client.options["show_fps"]:
            self.screen.blit(
                self.fonts["corbel-15"].render("FPS: {:.1f}".format(self.client.clock.get_fps()), True, RED),
                [self.screen.get_width() - 63, 0])
    ...
    def draw_blocks(self):
        for x in range(self.client.player.current_map[0] - 2, self.client.player.current_map[0] + 3):
            for y in range(self.client.player.current_map[1] - 1, self.client.player.current_map[1] + 2):
                ...
                if map:
                    self.screen.blit(map.image, [
                        map.draw_loc[0] - self.client.player.rect.x + (self.screen.get_rect().centerx - 15), \
    ...
    def draw_players(self):
        ...
        for name in self.client.players.names:
            if name != self.client.player_name:
                ...
                self.screen.blit(player.current_img,
                                 [player.rect.x - self.client.player.rect.x + (self.screen.get_rect().centerx - 15), \
        ...
        if self.client.player:
            ...
            self.screen.blit(self.client.player.current_img,
                             [self.screen.get_rect().centerx - 15, self.screen.get_rect().centery - 15])
        ...
        if self.client.show_list:
            for slot in range(len(self.client.players.names)):
                self.screen.blit(self.hud_textures["listslot"], \
                                 [(self.screen.get_width() / 2) - self.hud_textures["listslot"].get_width() / 2,
                                  (slot * 22) + 4])
                self.screen.blit(self.fonts["corbelb-15"].render(self.client.players.names[slot], True,
                                                                 self.client.players.names_color[
                                                                     self.client.players.names[slot]]), \
                                 [(self.screen.get_width() / 2) - self.hud_textures["listslot"].get_width() / 2 + 4,
                                  (slot * 22) + 8])
    ...
    def draw_projectiles(self):
        for identifier, data in self.client.projectiles.items():
            ...
            self.screen.blit(image,
                             [data["location"][0] - self.client.player.rect.x + (self.screen.get_rect().centerx - 15), \
    ...
    def load_block_textures(self):
        self.block_textures = {"dirt": None, "dirt_wall": None, "grass": None, "spike": None, "stone": None,
                               "test_block": None}
        for key in self.block_textures:
            try:
                self.block_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\" + self.texture_dir + "\\blocks\\" + key + ".png")
            except:
                self.block_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\default\\blocks\\" + key + ".png")
    ...
    def load_hud_textures(self):
        self.hud_textures = {"heart": None, "hearthalf": None, "inventoryslot": None, "slothighlight": None,
                             "hotbarslot": None, "listslot": None}
        for key in self.hud_textures:
            try:
                self.hud_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\" + self.texture_dir + "\\hud\\" + key + ".png")
            except:
    ...
    def load_projectile_textures(self):
        ...
        for key in self.projectile_textures:
            try:
                self.projectile_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\" + self.texture_dir + "\\projectiles\\" + key + ".png")
            except:
                self.projectile_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\default\\projectiles\\" + key + ".png")

    def load_item_textures(self):
        self.item_textures = {"wand_gold": None}
        for key in self.item_textures:
            try:
                self.item_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\" + self.texture_dir + "\\items\\" + key + ".png")
            except:
                self.item_textures[key] = pygame.image.load(
                    "..\\data\\textures\\packs\\default\\items\\" + key + ".png")

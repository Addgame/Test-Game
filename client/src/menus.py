...
def login(username_field, password_field):
    ...
    if len(password_field.get_text()) < 4:
        ...
        return
    event = pygame.event.Event(pygame.USEREVENT, name = "login", data = {"username":username_field.get_text(),"password":password_field.get_text()})
...
def connect(ip_field, port_field):
    ...
    event = pygame.event.Event(pygame.USEREVENT, name = "connect", data = {"ip":ip_field.get_text(),"port":port_field.get_text()})
    pygame.event.post(event)
    data_file = open("..\\data\\" + base64.b64encode("connection") + ".dat", 'w')
    data_file.write(base64.b64encode(json.dumps([ip_field.get_text(), port_field.get_text()], data_file)))
    data_file.close()

...
class ClientPlayerClass(pygame.sprite.Sprite):
    ...
    def load_images(self):
        try:
            self.images = {
                "player": pygame.image.load("..\\data\\textures\\players\\" + self.name + "\\player.png").convert(), \
                "playerCrouch": pygame.image.load(
                    "..\\data\\textures\\players\\" + self.name + "\\playerCrouch.png").convert(), \
                "playerCrouchJump": pygame.image.load(
                    "..\\data\\textures\\players\\" + self.name + "\\playerCrouchJump.png").convert(), \
                "playerDead": pygame.transform.rotate(
                    pygame.image.load("..\\data\\textures\\players\\" + self.name + "\\player.png"), 90).convert(), \
                "playerJump": pygame.image.load(
                    "..\\data\\textures\\players\\" + self.name + "\\playerJump.png").convert()}
        except:
            self.images = {"player": pygame.image.load("..\\data\\textures\\players\\player\\player.png").convert(), \
                           "playerCrouch": pygame.image.load(
                               "..\\data\\textures\\players\\player\\playerCrouch.png").convert(), \
                           "playerCrouchJump": pygame.image.load(
                               "..\\data\\textures\\players\\player\\playerCrouchJump.png").convert(), \
                           "playerDead": pygame.transform.rotate(
                               pygame.image.load("..\\data\\textures\\players\\player\\player.png"), 90).convert(), \
                           "playerJump": pygame.image.load(
                               "..\\data\\textures\\players\\player\\playerJump.png").convert()}
...
class ClientBlockItemClass(ClientBaseItemClass):
    ...
    def create_image(self):
        ...
        draw_loc = [self.image.get_width() - number_surface.get_width(),
                    self.image.get_height() - number_surface.get_height()]
...
class ClientInventoryClass():
    ...
    def items_from_list(self, item_list):
        ...
        for item_dict in item_list:
            ...
            item_obj = item_class(self.client, item_dict["count"], item_dict["internal_name"],
...
class ClientPlayerInventoryClass(ClientInventoryClass):
    ...
    def create_background_images(self):
        ...
        self.hotbar_background = pygame.Surface((36 * self.row_length, 36 * hb_num_row),
        ...
        for slot in range(self.num_slot):
            draw_loc = [(slot - (self.row_length * int(slot / self.row_length))) * 36,
                        self.full_background.get_height() - (ceil(float(slot + 1) / self.row_length) * 36)]
            if slot < self.hotbar_size:
                hb_draw_loc = [(slot - (self.row_length * int(slot / self.row_length))) * 36,
                               self.hotbar_background.get_height() - (ceil(float(slot + 1) / self.row_length) * 36)]
    ...
    def create_item_images(self):
        num_row = ceil(float(self.num_slot) / self.row_length)
        hb_num_row = ceil(float(self.hotbar_size) / self.row_length)
        self.hotbar_item_image = pygame.Surface((36 * self.row_length, 36 * hb_num_row),
                                                pygame.SRCALPHA).convert_alpha()
        ...
        for slot in range(self.num_slot):
            if self.items[slot] != self.client.NONE_ITEM:
                draw_loc = [(slot - (self.row_length * int(slot / self.row_length))) * 36 + 4,
                            self.full_item_image.get_height() - (ceil(float(slot + 1) / self.row_length) * 36) + 4]
                if slot < self.hotbar_size:
                    hb_draw_loc = [(slot - (self.row_length * int(slot / self.row_length))) * 36 + 4,
                                   self.hotbar_item_image.get_height() - (
                                   ceil(float(slot + 1) / self.row_length) * 36) + 4]
    ...
    def draw(self):
        ...
        self.client.graphics.screen.blit(self.client.graphics.hud_textures["slothighlight"], \
                                         [self.client.graphics.screen.get_width() - (36 * self.row_length) + ((
                                                                                                              self.selected_slot - (
                                                                                                              self.row_length * int(
                                                                                                                  self.selected_slot / self.row_length))) * 36), \
                                          self.client.graphics.screen.get_height() - 36 * int(
                                              ceil(float(self.selected_slot + 1) / self.row_length))])
